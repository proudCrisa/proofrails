...
This file defines how multiple AI agents should collaborate on this project using the ProofRails workflow.

## Agent roles

### Lead agent
- Invokes `/proofrails` for all structured work.
- Legacy aliases `/agentic-dev-harness` and `/agentic-dv-harness` are accepted for compatibility.
...
1. **Single entry.** The lead agent invokes ProofRails. Other agents are invoked by the lead, not by the user.
